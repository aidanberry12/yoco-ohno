...
def create_model(args,load_head=False):
    """Create a model
    """
    model_params = {'args': args, 'num_classes': 80}
    ...
    if model_path:  # make sure to load pretrained model
        ...
        if not load_head:
            ...
            del filtered_dict['head.decoder.duplicate_pooling']
            del filtered_dict['head.decoder.duplicate_pooling_bias']
            del filtered_dict['head.decoder.query_embed.weight']
